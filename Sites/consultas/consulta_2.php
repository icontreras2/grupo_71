...
<?php
  ...
  $comuna = $_POST["comuna_elegida"];
 	...
 	$query = "SELECT DISTINCT Trabajadores.id, Trabajadores.nombre, Trabajadores.rut, Trabajadores.edad, Trabajadores.sexo FROM Trabajadores, Tiendas, Direcciones, Comunas, TiendasTrabajadores WHERE TiendasTrabajadores.id_tienda = Tiendas.id AND Tiendas.id_jefe = TiendasTrabajadores.id_trabajador AND TiendasTrabajadores.id_trabajador = Trabajadores.id AND Tiendas.id_direccion = Direcciones.id AND Direcciones.id_comuna = Comunas.id AND Comunas.nombre = '$comuna';";
